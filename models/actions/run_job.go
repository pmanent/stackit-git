...
// >>> @@@ STACKIT CODE @@@
func GetRunJobsStatisticsThisMonth(ctx context.Context) (int64, int64, int64, int64, error) {
	now := time.Now().UTC()
	...
	_, err := db.GetEngine(ctx).
		...
		Where("started >= ?", t.Unix()).
		Where("stopped > started"). // exclude running jobs
		Select("SUM(stopped - started),AVG(stopped - started),MAX(stopped-started),count(*)").
		Get(&totalDuration, &averageDuration, &maxDuration, &countJobs)
	if err != nil {
		return 0, 0, 0, 0, err
	}
	return totalDuration, int64(averageDuration), maxDuration, countJobs, nil
}
