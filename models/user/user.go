...
// >>> @@@ STACKIT CODE @@@
// We have a problem with the logins of type user@domain.ext, they are not valid user format
// So we calculate a hash of the complete login to append to the leftmost part of the email.
// The return value is user.hash where hash is a 2 digits number calculated from the whole login
func GenerateCalculatedLogin(ctx context.Context, user string) (string, error) {
	parts := strings.Split(user, "@")
	if len(parts) != 2 {
		return "", fmt.Errorf("invalid user format: %s", user)
	}

	// First check is the user exists, and return it.
	var dbname string
	has, err := db.GetEngine(ctx).Table("user").Where("login_name = ?", user).Cols("name").Get(&dbname)
	if err != nil {
		return "", fmt.Errorf("get user existence: %s", user)
	}

	// The name exists and has the same login as the existing one, so nothing to create
	if has {
		return dbname, nil
	}

	// The name doesn't exists so just check the if the leftmost exists
	var login_name string
	has, err = db.GetEngine(ctx).Table("user").Where("name = ?", parts[0]).Cols("login_name").Get(&login_name)
	if err != nil {
		return "", fmt.Errorf("get user existence: %s", user)
	}

	// The name doesn't exist in the table so we can generate it
	if !has {
		return parts[0], nil
	}

	// If not, the name exists with another login name so generate a new one with 2 digits
	...
	// Convert the first  bytes of the hash to an integer
	hashInt := binary.BigEndian.Uint32(hashBytes[:4])

	// Ensure the hash is 2 digits long
	numericHash := int(hashInt % 100)

	ans := fmt.Sprintf("%s.%d", parts[0], numericHash)
