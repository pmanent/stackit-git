...
func main() {
	...
	// also load custom locale overrides so STACKIT-specific keys are recognised
	customLocaleFile := filepath.Join("custom", "options", "locale", "locale_en-US.ini")
	if customContent, readErr := os.ReadFile(customLocaleFile); readErr == nil {
		if err = localeiter.IterateMessagesContent(customContent, func(trKey, trValue string) error {
			msgids[trKey] = struct{}{}
			return nil
		}); err != nil {
			fmt.Printf("%s:\tERROR: %s\n", customLocaleFile, err.Error())
			os.Exit(2)
		}
	}
