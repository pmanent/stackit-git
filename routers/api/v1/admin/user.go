...
// >>> @@@ STACKIT Code
var (
	sendResetPasswordMailFunc  = mailer.SendResetPasswordMail
	sendRegisterNotifyMailFunc = mailer.SendRegisterNotifyMail
	adminCreateUserFunc        = user_model.AdminCreateUser
	passwordIsPwnedFunc        = password.IsPwned
)
...
func CreateUser(ctx *context.APIContext) {
	...
	if form.EmailNotificationsPreference != nil && *form.EmailNotificationsPreference != "" {
		u.EmailNotificationsPreference = *form.EmailNotificationsPreference
	}
	...
	if u.LoginType == auth.Plain {
		...
		if err := passwordIsPwnedFunc(ctx, form.Password); err != nil {
	...
	if err := adminCreateUserFunc(ctx, u, overwriteDefault); err != nil {
	...
	// >>> @@@ STACKITGIT-631
	if form.ForceSendResetPassword != nil && *form.ForceSendResetPassword {
		if err := sendResetPasswordMailFunc(ctx, u); err != nil {
			log.Error("Failed to send reset password mail to user %s: %v", u.Name, err)
		}

		// <<< @@@ STACKITGIT-631
		// Send email notification (only if the reset password mail has not been sent)
	} else if form.SendNotify {
		sendRegisterNotifyMailFunc(u)
	}
...
func EditUser(ctx *context.APIContext) {
	...
	opts := &user_service.UpdateOptions{
		...
		// >>> @@@ STACKIT CODE @@@
		EmailNotificationsPreference: optional.FromNilOrZero(form.EmailNotificationsPreference),
		// <<< @@@ STACKIT CODE @@@
	}
