...
func innerNewMainApp(version, versionExtra string, subCmdsStandaloneArgs, subCmdWithConfigArgs []*cli.Command, globalFlagsArgs []cli.Flag) *cli.App {
	app := cli.NewApp()
	app.HelpName = "stackitgit"
	app.Name = "STACKIT Git"
	app.Usage = ""
	app.Description = `By default, STACKIT Git will start serving using the web-server with no argument, which can alternatively be run by running the subcommand "web".`
