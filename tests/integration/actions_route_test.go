...
func testActionWorkflows(t *testing.T, session TestSession) {
	t.Run("sidebar exists", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		body.AssertElement(t, "#workflow_sidebar", true)
		assert.Contains(t, body.Find("#workflow_sidebar").Text(), "All workflows")
	})
	t.Run("sidebar show no workflows", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		workflowSidebar := body.Find("#workflow_sidebar")
		assert.Contains(t, workflowSidebar.Find("a").Text(), "All workflows")
		assert.Contains(t, workflowSidebar.Find("a").Text(), "No workflows")
	})
	t.Run("sidebar show workflows", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		workflowSidebar := body.Find("#workflow_sidebar")
		assert.Contains(t, workflowSidebar.Find("a").Text(), "All workflows")
		assert.NotContains(t, workflowSidebar.Find("a").Text(), "No workflows")
	})
	t.Run("main section exists", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		body.AssertElement(t, "#main_section", true)
	})
	t.Run("Actions are enabled", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		mainSection := body.Find("#main_section")
		assert.Contains(t, mainSection.Find("h3").Text(), "STACKIT Git Actions are now enabled.")
	})
}

func testActionsRunners(t *testing.T, session TestSession) {
	t.Run("Stackit runners not defined", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		mainSection := body.Find("#main_section")
		assert.Contains(t, mainSection.Find("h4").Text(), "Enable Stackit runners")
	})
	t.Run("Stackit runners are defined", func(t *testing.T) {
		body := getActionsPageBody(t, session)
		mainSection := body.Find("#main_section")
		assert.NotContains(t, mainSection.Find("h4").Text(), "Enable Stackit runners")
	})
}
