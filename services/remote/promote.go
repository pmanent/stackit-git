...
func GetUsersByLoginName(ctx context.Context, name, calculatedLogin, loginName string) ([]*user_model.User, error) {
	...
	// trick to get new logins and legacy one's
...
func GetUsersByMail(ctx context.Context, emails ...string) ([]*user_model.User, error) {
	if len(emails) == 0 {
		return nil, user_model.ErrUserNotExist{Name: "nil"}
	}
