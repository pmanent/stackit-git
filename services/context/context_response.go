...
// @@@ STACKIT
// Redirect to login page with a given redirect URL
func (ctx *Context) RedirectToLogin(redirectURL string) {
	redirectURL = path.Join(setting.AppSubURL, "/user/login?redirect_to="+url.QueryEscape(redirectURL))
	log.Trace("Redirect to login: %s", redirectURL)
