...
func newRequest(ctx context.Context, method, url string, body io.ReadCloser) (*http.Request, error) {
	...
	// Since the request below is exposing the Gitea version, we decided to change it to hold an obfuscated value.
	// Currently, the flag that checks the password (PASSWORD_CHECK_PWN) does not have an assigned value (defaults to false)
	// so this change shuldn't have any consequences, since we aren't using this service.
