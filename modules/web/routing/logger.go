...
func logPrinter(logger log.Logger) func(trigger Event, record *requestRecord) {
	return func(trigger Event, record *requestRecord) {
		...
		latency := time.Since(record.startTime)

		logf("router: %s %v %s for %s, %v %v in %v @ %s",
			message,
			log.ColoredMethod(req.Method), req.RequestURI, req.RemoteAddr,
			log.ColoredStatus(status), log.ColoredStatus(status, http.StatusText(status)), log.ColoredTime(latency),
			handlerFuncInfo,
		)

		// Latency is expressed in milliseconds
		stats.Add(float64(latency.Milliseconds()), status)
